...
namespace ImplThread{
    ...
    void stopAllThreads(){
        ...
        for ( std::map<const pthread_t, int*>::iterator it = mymap.begin(); it != mymap.end(); it++){
            pthread_t thread_id = it->first; // element stored in the map as [first, second], first is thread_id, second is stop flag
            *(it->second) = 1; // Set the stop flag as 1
            ...
            if(pthread_timedjoin_np(thread_id, NULL, &ts))
                pthread_cancel(thread_id); // Cancel the thread by thread_id
//             it=mymap.begin();
        }
