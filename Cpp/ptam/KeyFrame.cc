...
// #include "Hessian/trhessianfeaturedetector.h"
...
void KeyFrame::MakeKeyFrame_Lite(BasicImage<byte> &im)
{
  ...
  for(int i=0; i<LEVELS; i++)
	...
	{  // .. make a half-size image from the previous level..
	  ...
	  //cv::Mat image( lev.im.size().y , lev.im.size().x , CV_8UC1 );
	 ...
	 // std::cout<<image.rows<<" "<<image.cols<<std::endl;
	  ...
	  //memcpy( image.data , lev.im.data() , image.rows * image.cols );
      ...
      if(i == 3)
	   ...
	   //tr::TrHessianFeatureDetector detector;
   ...
   //   if(i == 3)
	  ...
	  //std::cout<<" feature detection completed "<<numKeyPoints<<std::endl;

	  //lev.vCorners.resize( numKeyPoints );

	  //for( int kk = 0; kk < numKeyPoints ; kk++ )
	  //{
		 // lev.vCorners[ kk ].x = keyPoints[ kk ].pt.x;
		 // lev.vCorners[ kk ].y = keyPoints[ kk ].pt.y;
	  //}

	 // std::cout<<i<<" -- "<<lev.vCorners.size()<<std::endl;
