...
void MapViewer::getDrawData( std::vector< QVector3D > &points , std::vector< QVector3D > &cameras )
{
  
  points.resize( mMap.vpPoints.size() );
  
  cameras.resize( mMap.vpKeyFrames.size() );
  
  for( size_t i = 0; i < mMap.vpKeyFrames.size() ; i++ )
  {
     Vector< 3 > v3Pos = mMap.vpPoints[i]->v3WorldPos;
     
     points[ i ] = QVector3D( v3Pos[ 0 ] , v3Pos[ 1 ] , v3Pos[ 2 ] );
  }
  
}

bool MapViewer::isRedrawNeeded( int prevNumCams )
{
    return mMap.vpKeyFrames.size() > prevNumCams;
}
