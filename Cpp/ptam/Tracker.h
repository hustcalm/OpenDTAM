...
public:
  ...
  void TrackFrameCustom( CVD::Image<CVD::byte> &imFrame , QString &message , bool &spaceBarPressed ,
                         std::vector< QPoint > &fastCorners , std::vector< QPoint > &trackedPoints ,  std::vector< QVector3D > &objectPoints , std::vector< std::pair< QPoint , QPoint > > &trails , QMatrix4x4 &pose  );
...
//#ifdef VE_UNIX
  void TrackForInitialMap( std::vector< std::pair< QPoint , QPoint > > &trails , QString &message );      // This is called by TrackFrame if there is not a map yet.
  int TrailTracking_Advance( std::vector< std::pair< QPoint , QPoint > > &trails );
  void TrackMapCustom( std::vector< QPoint > &trackedPoints , std::vector< QVector3D > &objectPoints , QMatrix4x4 &pose );
//#endif
  ...
  TooN::Vector<6> CalcPoseUpdate(std::vector<TrackerData*> vTD, 
