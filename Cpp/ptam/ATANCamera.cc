...
ATANCamera::ATANCamera( Vector< 5 > &params , int &width , int &height )
{
  ...
  mvFocal[0] = mvImageSize[0] * params[0];
  mvFocal[1] = mvImageSize[1] * params[1];
  mvCenter[0] = mvImageSize[0] * params[2] - 0.5;
  mvCenter[1] = mvImageSize[1] * params[3] - 0.5;

  std::cout<<" camera intrinsics : "<<mvFocal[0]<<" "<<mvFocal[1]<<" "<<mvCenter[0]<<" "<<mvCenter[1]<<std::endl;
  ...
  Vector<2> v2;
  v2[0]= max(params[2], 1.0 - params[2]) / params[0];
  v2[1]= max(params[3], 1.0 - params[3]) / params[1];
