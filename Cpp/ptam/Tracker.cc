...
//#ifdef VE_UNIX
void Tracker::TrackFrameCustom( Image< byte >& imFrame, QString& message, bool &spaceBarPressed,
							   std::vector< QPoint >& fastCorners, std::vector< QPoint >& trackedPoints , std::vector< QVector3D > &objectPoints , 
				std::vector< std::pair< QPoint, QPoint > >& trails  , QMatrix4x4 &pose   )
{
//   mbDraw = bDraw;
//   mMessageForUser.str("");   // Wipe the user message clean
  
  mbUserPressedSpacebar = spaceBarPressed;
...
//   std::cout<<" sbi blurr : "<<*gvdSBIBlur<<std::endl;
...
//     }
    ...
    fastCorners.resize( mCurrentKF.aLevels[0].vCorners.size() );
  
    for( unsigned int i=0; i < mCurrentKF.aLevels[0].vCorners.size(); i++) 
    { 
      fastCorners[ i ] = QPoint( mCurrentKF.aLevels[0].vCorners[ i ].x , mCurrentKF.aLevels[0].vCorners[ i ].y );
    }
  ...
  if(mMap.IsGood())
      ...
      if(mnLostFrames < 3)  // .. but only if we're not lost!
	{
	  ...
	  //TrackMap();               //  These three lines do the main tracking work.
	  TrackMapCustom( trackedPoints , objectPoints , pose );
	  ...
	  { // Provide some feedback for the user:
	    message += "Tracking Map, quality ";
	    if(mTrackingQuality == GOOD)  message += "good.";
	    if(mTrackingQuality == DODGY) message += "poor.";
	    if(mTrackingQuality == BAD)   message += "bad.";
	    message += " Found:";
	    for(int i=0; i<LEVELS; i++) message += " " + QString::number( manMeasFound[i] )  + "/" + QString::number( manMeasAttempted[i] );
	    //	    mMessageForUser << " Found " << mnMeasFound << " of " << mnMeasAttempted <<". (";
	    message +=  " Map: " + QString::number( mMap.vpPoints.size() ) + "P, " + QString::number( mMap.vpKeyFrames.size() ) + "KF";
	  }
...
void Tracker::TrackForInitialMap()
{
  ...
  if(mnInitialStage == TRAIL_TRACKING_STARTED)
      ...
      if(nGoodTrails < 10) // if most trails have been wiped out, no point continuing.
	{
	  ...
	  std::cout<<" num good trails "<<nGoodTrails<<std::endl;
...
//#ifdef VE_UNIX
void Tracker::TrackForInitialMap(std::vector< std::pair< QPoint , QPoint > > &trails , QString &message)
{
    // MiniPatch tracking threshhold.
  static gvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD", 100000, SILENT);
  MiniPatch::mnMaxSSD = *gvnMaxSSD;
  ...
  if(mnInitialStage == TRAIL_TRACKING_NOT_STARTED) 
    ...
    //mMessageForUser << "Point camera at planar scene and press spacebar to start tracking for initial map." << endl;
          message += "Point camera at planar scene and press spacebar to start tracking for initial map.";
      return;
    };
  
  if(mnInitialStage == TRAIL_TRACKING_STARTED)
    {
      int nGoodTrails = TrailTracking_Advance( trails );  // This call actually tracks the trails
      if(nGoodTrails < 10) // if most trails have been wiped out, no point continuing.
	{
	  Reset();
	  return;
    ...
    message +=  "Translate the camera slowly sideways, and press spacebar again to perform stereo init.";// << endl;
    }
}
//#endif
...
int Tracker::TrailTracking_Advance( std::vector< std::pair< QPoint , QPoint > > &trails )
{
  ...
  for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end();)
      ...
      if(bFound)
	{
      ...
      /*if(mbDraw)
	{*/	  
	  std::pair< QPoint , QPoint > trailPair;
	  
	  trailPair.first.rx() = trail.irInitialPos.x;
	  trailPair.first.ry() = trail.irInitialPos.y;
	  
	  trailPair.second.rx() = trail.irCurrentPos.x;
	  trailPair.second.ry() = trail.irCurrentPos.y;
	  
	  trails.push_back( trailPair );
// 	}
...
void Tracker::TrackMap()
{
  ...
  if(mbDraw)
	  ...
	  it++)
	{
	  ...
	  Rgb< byte > color(gavLevelColors[(*it)->nSearchLevel][ 0 ] , gavLevelColors[(*it)->nSearchLevel][ 1 ] , gavLevelColors[(*it)->nSearchLevel][ 2 ]);
	  glColor(color);
	  glVertex((*it)->v2Image);
	}
      glEnd();
...
void Tracker::TrackMapCustom( std::vector< QPoint > &trackedPoints, std::vector< QVector3D > &objectPoints  , QMatrix4x4 &pose )
{
  // Some accounting which will be used for tracking quality assessment:
  for(int i=0; i<LEVELS; i++)
    manMeasAttempted[i] = manMeasFound[i] = 0;
  ...
  SE3<> relPose = mse3CamFromWorld ;
  
  TooN::Matrix< 3 , 3 > rot = relPose.get_rotation().get_matrix();
  TooN::Vector< 3  > trans =  relPose.get_translation(); 

  for( int ii = 0; ii < 3; ii++ )
  {
	  for( int jj = 0; jj < 3; jj++ )
	  {
		  pose( ii , jj ) = rot[ ii ][ jj ];
	  }

	  pose( ii , 3 ) = trans[ ii ];
  }

//   std::cout<<" printing transformations "<<std::endl;
...
//    {
	  ...
	  it++)
	{
	  ...
	  trackedPoints.push_back( QPoint( (*it)->v2Image[ 0 ] , (*it)->v2Image[ 1 ] ) );

	  objectPoints.push_back( QVector3D( (*it)->Point.v3WorldPos[ 0 ] , (*it)->Point.v3WorldPos[ 1 ] , (*it)->Point.v3WorldPos[ 2 ] )  );
	}
//    }
